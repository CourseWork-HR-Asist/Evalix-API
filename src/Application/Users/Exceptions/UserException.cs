...
public class UserLoginWithGoogleExistsException(string email)
    : UserException(UserId.Empty(), $"User with email already exists: {email}");
    
public class UserLoginWithGoogleNotAcceptedException(string email)
    : UserException(UserId.Empty(), $"User with email not accepted login with google auth: {email}");
